...
export const useCart = create<CartStore>((set) => ({
   cartProducts: [],
   addProduct: (cartProduct) => set(
      ({ cartProducts }) => ({ cartProducts: [...cartProducts, cartProduct] }
   )),
   increaseProduct: (cartProductId) => {
      set(({ cartProducts }) => ({
         cartProducts: cartProducts
            .map(
               cardProduct => cardProduct.product.id === cartProductId ? ({
                  ...cardProduct,
   ...
   decrementProduct: (cartProductId) => {
      set(({ cartProducts }) => {
        const d: ProductCart[] = cartProducts
            .map(
               cardProduct => {
                  if(cardProduct.quantity === 1 && cardProduct.product.id === cartProductId){
                     return null;
                  }
   ...
   removeProduct: (cartProductId) => {
      set(({ cartProducts }) => ({
         cartProducts: cartProducts.filter(cartProduct => cartProduct.product.id !== cartProductId)
      }))
   }
}))